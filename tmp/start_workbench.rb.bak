...
def generate_command(pwd, h_n, u_n)

	paths = pwd.split('/')
	platform_path = paths[-1]
	separated_paths = separate_path(platform_path)
	fail "can not get platform paths by" + pwd if separated_paths.empty?
...
fail "can not get $HOSTNAME" unless host_name
user_name = ENV["USERNAME"]
fail "can not get $USERNAME" unless user_name
#system(generate_command(pwd, host_name, user_name))
